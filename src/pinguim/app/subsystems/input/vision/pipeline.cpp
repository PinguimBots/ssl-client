...
cv::Mat Pipeline::execute(pinguim::vsss::game_info& gi, cv::Mat const& currframe, pinguim::app::subsystems::input::vision_impl::Colors colors)
{
    ...
    if(contours.ballContour.size())   cv::drawContours(frame2, contours.ballContour,   -1, {0, 255, 0}, 3, cv::LINE_AA);
    for(const auto& role : contours.rolesContour)
        if(role.size()) cv::drawContours(frame2, role, -1, {255, 255, 0}, 3, cv::LINE_AA);

    ImGui::Begin("Contours");
    pb::ImGui::Image({frame2, GL_BGR, GL_RGB});
    ImGui::End();
