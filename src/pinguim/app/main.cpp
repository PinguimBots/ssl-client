...
auto draw_env_packet_window(auto args, auto env_packet, auto& show_window)
{
    if( ImGui::Begin(fmt::format("Environment Packet (received from {}:{})", args.in_address, args.in_port).c_str(), &show_window) )
    {
        ImGui::BeginGroup();
        {
...
int main(int argc, char *argv[])
{
    ...
    auto strategy_vis_img = pinguim::imgui::img{ strategy_vis };
    ...
    while(!quit){
        ...
        if(show_env_packet_window) { draw_env_packet_window(args, env_packet, show_env_packet_window); }
        ...
        if(show_lena_window)
        {
            if(ImGui::Begin("Lena (OpenGL/OpenCV test)", &show_lena_window))
            { ImGui::Image(lena); }
            ImGui::End();
        }

        if(!new_env) {goto endframe;}
